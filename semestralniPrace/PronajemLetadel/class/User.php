...
class User
{
    ...
    public static function update($pid,$pusername,$pheslo,$pjmeno,$pprijmeni,$pemail,$ptelefon) {
        try {
            ...
            if (empty($pheslo)) {
                ...
                $heslo = password_hash($pheslo,PASSWORD_BCRYPT);
            }
    ...
    public static function getAll($id) {
        try {
            $conn = Connection::getPdoInstance();
            $stmt = $conn->prepare("
SELECT id_uzivatel, username, jmeno, prijmeni, email, telefon 
FROM uzivatel 
WHERE id_uzivatel <> :id
AND  id_uzivatel NOT IN (
  SELECT uz.id_uzivatel 
  FROM uzivatel uz
  JOIN role r
  ...
  WHERE op.nazev = 'privatni sprava')
